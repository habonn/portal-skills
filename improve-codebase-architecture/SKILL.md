...
description: |
  ...
  USE FOR: improving architecture, finding refactoring opportunities,
  consolidating tightly-coupled modules, making code more testable and
  AI-navigable.
  DO NOT USE FOR: writing new features, debugging, quick fixes.
---
...
This skill is informed by the project's domain model. The domain language gives
...
Then walk the codebase. Don't follow rigid heuristics — explore organically and
...
Use CONTEXT.md vocabulary for the domain. If `CONTEXT.md` defines "Order," talk
about "the Order intake module" — not "the FooBarHandler."

ADR conflicts: if a candidate contradicts an existing ADR, only surface it when
the friction is real enough to warrant revisiting the ADR. Mark it clearly.
